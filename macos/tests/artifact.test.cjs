...
const artifact={name:'macos-universal-dmg-signed',expired:false,size_in_bytes:100,expires_at:'2099-01-01T00:00:00Z'};
async function select(runs,artifacts){
 ...
 const github={rest:{actions:{listWorkflowRuns:'runs',listWorkflowRunArtifacts:'artifacts'}},paginate:async(method,args)=>{
  if(method==='runs'){assert.equal(args.head_sha,context.sha);assert.equal(args.workflow_id,'macos.yml');return runs;}
...
test('reuses only completed signed artifacts and falls back past newer runs without one',async()=>{
...
test('missing, expired, unsigned and empty artifacts require a fresh build',async()=>{
 for(const artifacts of [[],[{...artifact,expired:true}],[{...artifact,name:'macos-universal-dmg'}],
