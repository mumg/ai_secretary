...
## Вход из админки Improver (2026-09-16)

В карточке МТС Линк и форме нового источника доступна кнопка «Войти через SSO».
Новый источник сначала сохраняется выключенным; после успешного входа включается,
если включение было выбрано в форме. При отмене он остаётся выключенным.

Для облачного gateway используется расширение Chrome/Edge из архива, который
админка отдаёт по `GET /api/v1/admin/mts-link/extension.zip`. Исходники расширения:
`browser-extension/src/`. Команда `python3 browser-extension/build.py` создаёт
`dist/ai-secretary-extension.zip` и копию для упаковки backend. Docker собирает
архив автоматически; подробности — в `browser-extension/README.md`.

Расширение называется «AI Секретарь», иконка взята из `assets/secretary-robot.svg`.
Настройки проверяют доступность расширения при открытии и возврате в окно.
Проверка проходит через content script к service worker: отключённое расширение
не считается готовым. При отсутствии ответа предлагаются ввод access token и
установка расширения; после подключения становится доступна кнопка SSO.
С разрешением `activeTab` установленное расширение нужно активировать нажатием
значка на странице; до этого оно также отображается как не обнаруженное.
Для ручного токена показан заданный срок 84 часа с момента выдачи, не сохранения.
Поле токена остаётся доступным и при подключённом расширении.

1. Скачать и распаковать архив, открыть `chrome://extensions` / `edge://extensions`.
2. Включить режим разработчика и загрузить распакованное расширение.
3. На странице `/admin` нажать значок расширения; повторять после перезагрузки админки.
4. Нажать «Войти через SSO», ввести рабочий email и выбрать организацию.
5. Выполнить вход и MFA в отдельном окне корпоративного провайдера.
6. Окно закроется; карточка покажет, что настроено автоматическое обновление токенов.

Расширение использует `activeTab`, а не постоянный доступ ко всем сайтам.
Оно подключает `chrome.debugger` только к созданной им вкладке входа и перехватывает
HTTP `Location: mtslink://mobile/login?authCode=...` на стадии ответа, до передачи
схемы приложению. `authCode` возвращается только исходной вкладке админки, если её
origin и путь всё ещё соответствуют разрешённой странице. Окно закрывается также
по отмене или через 15 минут. Включённую во время входа панель отладки Chrome
не следует отключать.

Backend сам получает способы SSO по email, формирует URL для SAML/OAuth и выдаёт
зашифрованный билет, действующий 15 минут и привязанный к источнику и его настройкам.
Полученный код обменивается через `AccountUcaas.LoginByAuthCode`; access/refresh
не возвращаются в ответы админки. Их пара хранится в существующем зашифрованном
поле источника. Миграция БД не нужна, прежние одиночные access token поддерживаются.

При 401/403 коннектор обновляет пару через `AccountUcaas.Refresh`, сохраняет её
отдельной транзакцией и один раз повторяет запрос. Блокировка строки источника в
PostgreSQL предотвращает одновременное использование одного refresh token разными
процессами. Устаревший экземпляр коннектора использует уже сохранённый access token.
Замена токена вручную удаляет refresh token, чтобы не смешать разные аккаунты.

Полный путь расширение → popup → HTTPS 302 → админка проверен в Chrome for Testing
с локальным SSO-сервером и синтетическим кодом. Вход пользователя и реальная ротация
на стороне МТС Линк требуют установки расширения и прохождения SSO пользователем.
Политики корпоративного браузера могут запрещать unpacked extensions/debugger.

### Почему нужен компонент браузера

Проверка начала SAML-входа показала: `returnUrl=https://localhost/...` и
`returnUrl=http://127.0.0.1:8000/...` отклоняются с HTTP 400 `Not owned returnURL`;
`returnUrl=mtslink://mobile/login` принят с HTTP 302 на корпоративный IdP.
Обычный JS админки не может прочитать чужой popup из-за same-origin policy,
а `registerProtocolHandler('mtslink', ...)` запрещён браузером.

### Официальные альтернативы

[OAuth для партнёрских приложений](https://help.mts-link.ru/article/21125) поддерживает
зарегистрированный HTTPS callback, access/refresh и `grant_type=refresh_token` через
`https://my.mts-link.ru/api/idp/oauth/token`. Нужны партнёрское приложение,
`client_id`, `client_secret`, разрешение администратора организации и тариф с API.
В документации для чтения расшифровок указан scope `userapi_events_read`.

[API-ключ](https://help.mts-link.ru/article/19686) передаётся как `x-auth-token` в
UserAPI. [Ограничения прав](https://help.mts-link.ru/article/21127) позволяют выбрать
чтение событий/расшифровок. Документированы
[список расшифровок](https://help.mts-link.ru/article/20038) и
[получение текста](https://help.mts-link.ru/article/19230), в том числе для постоянных
встреч. UserAPI использует другие endpoint/контракты, чем текущий gateway-коннектор;
нельзя просто подставить API-ключ либо партнёрский OAuth-токен в его поле access token.

Для текущей установки пользователь подтвердил, что доступен только личный
корпоративный SSO, без API-ключа организации и регистрации OAuth-приложения.
Поэтому реализован вход через расширение, а переход на UserAPI не выполнялся.
