...
func Migrate(ctx context.Context, pool *pgxpool.Pool) error {
	...
	if len(revisions) == 1 {
		...
		if e != nil || current < 1 || current > 22 {
			return errors.New("unsupported database revision")
		}
	}
	for n := current + 1; n <= 22; n++ {
	...
	if _, e = tx.Exec(ctx, "INSERT INTO database_schema_version (id,version,revision) VALUES (1,22,'0022') ON CONFLICT(id) DO UPDATE SET version=22,revision='0022',updated_at=now()"); e != nil {
		return e
	}
