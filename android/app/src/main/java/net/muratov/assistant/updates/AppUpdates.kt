...
class AppUpdates(private val context: Context) {
    ...
    suspend fun check(force: Boolean = false): Boolean = withContext(Dispatchers.IO) {
        mutex.withLock {
            ...
            mutableState.value = mutableState.value.copy(busy = true, message = "Проверяем обновления…")
            try {
                client.newCall(Request.Builder().url(UPDATE_MANIFEST_URL).build()).execute().use { response ->
                    if (response.code == 404) {
                        mutableState.value = UpdateState(message = "Публичных Android-сборок пока нет")
                    } else {
                        if (!response.isSuccessful) throw IOException("HTTP ${response.code}")
                        val source = response.body.source()
                        source.request(65_537)
                        require(source.buffer.size <= 65_536) { "Слишком большой манифест обновления" }
                        val text = source.buffer.readUtf8()
                        val release = decode(text)
                        if (release.versionCode <= BuildConfig.VERSION_CODE) {
                            mutableState.value = UpdateState(message = "Установлена актуальная версия")
                            ...
                            mutableState.value = UpdateState(release = release, ready = apk(release).isFile,
                                message = "Доступна версия ${release.versionName}")
                        }
                    }
                }
                ...
                mutableState.value = mutableState.value.copy(busy = false, message = "Не удалось проверить обновление: ${failure.message.orEmpty().take(140)}")
                false
    ...
    private suspend fun downloadAutomatically() {
        ...
        else mutableState.value = mutableState.value.copy(message = "Обновление ждёт Wi-Fi. Можно скачать вручную.")
    }
    ...
    suspend fun download() = withContext(Dispatchers.IO) {
        mutex.withLock {
            try { downloadLocked()
                ...
                mutableState.value = mutableState.value.copy(message = "Не удалось скачать обновление: ${failure.message.orEmpty().take(140)}")
            } finally { mutableState.value = mutableState.value.copy(busy = false) }
        }
    }
    ...
    private suspend fun downloadLocked() {
        ...
        mutableState.value = mutableState.value.copy(busy = true, message = "Скачиваем ${release.versionName}…")
        try {
            client.newCall(Request.Builder().url(release.apkUrl).build()).execute().use { response ->
                ...
                response.body.byteStream().use { input -> partial.outputStream().use { output ->
                    ...
                    while (true) {
                        ...
                        if (length < 0) break
                        count += length
                        require(count <= release.size) { "Размер APK не совпадает" }
            ...
            mutableState.value = mutableState.value.copy(busy = false, ready = true, message = "Обновление готово к установке")
            notifyReady(release)
    ...
    private fun verify(file: File, release: UpdateManifest) {
        try {
            require(file.length() == release.size) { "APK загружен не полностью" }
            val digest = MessageDigest.getInstance("SHA-256")
            file.inputStream().use { input ->
                val buffer = ByteArray(64 * 1024)
                while (true) { val size = input.read(buffer); if (size < 0) break; digest.update(buffer, 0, size) }
            }
            require(digest.digest().joinToString("") { "%02x".format(it) } == release.sha256) { "Контрольная сумма APK не совпадает" }
            val flags = if (Build.VERSION.SDK_INT >= 28) PackageManager.GET_SIGNING_CERTIFICATES else PackageManager.GET_SIGNATURES
            val installed = context.packageManager.getPackageInfo(context.packageName, flags)
            val archive = context.packageManager.getPackageArchiveInfo(file.path, flags) ?: error("Некорректный APK")
            require(archive.packageName == context.packageName && PackageInfoCompat.getLongVersionCode(archive) == release.versionCode &&
                release.versionCode > PackageInfoCompat.getLongVersionCode(installed)) { "Пакет или версия APK не совпадают" }
            ...
            require(trusted.isNotEmpty() && signatures(archive) == trusted) {
                "APK подписан другим ключом. Для перехода с debug нужна установка release-сборки."
            }
        } catch (failure: Exception) {
            file.delete()
    ...
    suspend fun installIntent(): Intent = withContext(Dispatchers.IO) {
        mutex.withLock {
            val release = state.value.release ?: error("Нет обновления")
    ...
    private fun notifyReady(release: UpdateManifest) {
        ...
        manager.notify(4501, NotificationCompat.Builder(context, "app_updates")
            ...
            .setContentTitle("AI Секретарь ${release.versionName}")
            .setContentText("Обновление скачано. Откройте приложение для установки.")
