...
data class UpdateManifest(
    ...
    fun validate(packageName: String, sdk: Int): UpdateManifest {
        require(versionCode > 0 && Regex("[0-9]+\\.[0-9]+\\.[0-9]+").matches(versionName)) { "Некорректная версия обновления" }
        require(applicationId == packageName) { "Обновление предназначено для другого приложения" }
        require(minSdk in 26..sdk) { "Для обновления нужна более новая версия Android" }
        require(Regex("[a-f0-9]{64}").matches(sha256)) { "Нет корректной контрольной суммы" }
        require(size in 1..200_000_000) { "Некорректный размер APK" }
        val uri = URI(apkUrl)
