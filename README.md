...
Скриншоты ниже обновлены **21.09.2026** для версии **0.7.19** и сняты
с реализованного интерфейса на **синтетических данных**.
...
![WEB: очередь обработки и состояние компонентов](assets/web/status.png)
...
только для локальной диагностики сервера. Android debug- и release-сборки используют HTTPS.
Адрес сервера и клиентский сертификат для mTLS импортируются камерой из QR-кода
...
При первом запуске мастер сканирует QR-код из админской страницы камерой,
получает адрес и ключ доступа, затем проверяет подключение.
...
## Предконфигурация для корпоративной установки

Источники, LLM и начальные настройки можно доставить файлом `secretary-config.json`
рядом с приложением. Пользователь дополняет и переопределяет их в админке; в БД
сохраняются только отличия. Поддерживаются Windows, macOS и Docker.
[Формат JSON и установка через политики](PRECONFIGURATION.md).

Ollama можно [установить дополнительно из настроек](OLLAMA.md) на Windows и macOS:
с проверкой компьютера, загрузкой модели и явным переключением рабочего LLM.

### Политики мобильных уведомлений

В «Настройки → Уведомления» доступны режимы «Сразу» (объединение за 2 минуты,
минимум 5 минут между отправками), «Сводками каждые 15 минут» и «Только важное».
Важными считаются новые и критические задачи, запросы подтверждения, напоминания,
приближение срока и просрочка. Ограничение частоты действует и для важных задач.
Несколько изменений одной задачи объединяются; несколько объектов дают один
общий push, открывающий приложение. Содержимое сводки проверяется на актуальность
перед отправкой. Закрытые задачи и отменённые встречи исключаются.

Тихие часы используют `server.timezone`; по умолчанию выключены, предложенный
интервал — 22:00–08:00. После тихих часов отправляется общая сводка накопленного.
Ежедневную сводку можно отключить: это отключает push о готовности плана и
ежедневные повторы просрочек, сохраняя первое уведомление о просроченной задаче.

События с датой не позднее момента добавления источника обрабатываются без push,
включая итоговую сводку и напоминания по задачам из этой истории. Новые события
после добавления источника могут вызывать push даже во время первой загрузки.
Задержка загрузки или анализа более чем на сутки не делает новое событие историей.
Повторный анализ не создаёт уведомлений. Heartbeat и другие
технические события не создают push. Активное приложение продолжает получать
изменения через существующее защищённое соединение независимо от политики push.

Настройки в `secretary-config.json`, в секции `notifications`:
...
Очередь и результаты отправки по устройствам хранятся в PostgreSQL и сохраняются
при перезапуске. При включённом шлюзе используется только маршрут через шлюз,
чтобы не дублировать прямую отправку Firebase. Повтор запроса к шлюзу использует
тот же идентификатор; успешные устройства повторно не отправляются. Срок хранения
push у провайдера — до 5 минут и не позднее начала тихих часов. Истёкшая неуспешная
отправка заменяется актуальной сводкой. Принятие push провайдером не является
подтверждением показа или прочтения на телефоне; отдельного учёта прочтения пока нет.


### Глубина создания задач при инициализации источника

В форме добавления источника поле «Глубина создания задач и поручений, дней»
задаёт период истории для автоматического создания новых задач и поручений.
Допустимы целые значения от 0 до 365. Значение 0 разрешает создание только из
событий после добавления источника. Для нового источника по умолчанию используется
текущая глубина загрузки истории (`communication_sources.initial_sync_days`, обычно 30).

Граница фиксируется относительно даты добавления источника: при глубине 7 дней
сообщения старше этой границы остаются в архиве и поиске, но не создают новые
назначения. Правило сохраняется при поздней загрузке, перезапуске и повторном
анализе. Более новые события обрабатываются как обычно. Обновления уже созданных
задач и поручений не блокируются; изменение глубины не удаляет существующие записи
и не сбрасывает курсор загрузки. Явно переданные задачи через внешний API не
ограничиваются этим параметром автоматического анализа.

В `secretary-config.json` параметр задаётся отдельно для каждого источника:
`sources[].settings.initial_assignment_days`, например `7`.
У существующих и преднастроенных источников без параметра сохраняется прежний
анализ всей загруженной истории; в форме такого источника поле можно оставить
пустым. Глубина фактической загрузки архива задаётся отдельно и может быть меньше
глубины создания задач. Push для событий до добавления источника по-прежнему
не отправляются, даже если из этих событий созданы задачи.
