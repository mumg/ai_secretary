...
сервере. Qwen работает через вашу Ollama, без отправки рабочего контекста внешнему
LLM-провайдеру.

Доступ к API защищён TLS и обязательным клиентским сертификатом для каждого
Android-устройства. Пароли источников и серверный ключ Firebase хранятся в базе в
...
встреч. Анализ выполняется локальной моделью Qwen через отдельно запущенную Ollama;
...
4. Импортируйте сгенерированный клиентский сертификат и откройте
   ...
   Ollama, уведомления и все источники переписки. YAML-конфигурация приложению
   больше не требуется.
...
Адаптер МТС Линк использует reverse-engineered контракт из `MTS_LINK.md` и
`mts-link.openapi.yaml`. В веб-панели для него задаются `https://gw.mts-link.ru` и действующий
access token. Endpoint обновления токена в наблюдаемом протоколе отсутствует, поэтому после
истечения токена его нужно заменить вручную; состояние видно в карточке источника.
Интервал опроса задаётся отдельно для источника; для рабочего источника МТС Линк используется
900 секунд.
