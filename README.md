...
Включает Go-помощник установки, локальный PostgreSQL, фоновые службы
...
и ключи. Сборка и проверка установки выполняются GitHub Actions.
...
На странице `/admin/` → «Мобильное приложение» можно выпустить отдельный
клиентский ключ и передать его приложению через QR-код. Нужны публичный HTTPS
